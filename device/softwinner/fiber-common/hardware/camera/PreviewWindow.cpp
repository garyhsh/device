...
static int calculateFrameSize(int width, int height, uint32_t pix_fmt)
{
	...
	switch (pix_fmt) {
		...
		case V4L2_PIX_FMT_NV12:
			frame_size = (ALIGN_32B(width) * height * 12) / 8;
			break;
...
bool PreviewWindow::onNextFrameAvailable(const void* frame)
{
    ...
    if (adjustPreviewDimensions(pv4l2_buf) || mShouldAdjustDimensions) {
		...
		switch (pv4l2_buf->format)
		{
			...
			case V4L2_PIX_FMT_YUYV:
				...
				break;
			default:
				LOGE("preview unknown pixel format: %08x", pv4l2_buf->format);
				return false;
		}
		...
		|| preview_format == V4L2_PIX_FMT_NV12)
	{
		...
		for (int h = 0; h < preview_height; h++)
		{
			...
			src += ALIGN_32B(preview_width);
			dst += ALIGN_16B(preview_width);
		}
		// uv
	...
	else if(preview_format == V4L2_PIX_FMT_YUV420)
	{
		// YU12 to YV12
		// y
		memcpy(img, (void*)preview_addr_vir, ALIGN_32B(preview_width) * preview_height);

		// u
		memcpy(img + ALIGN_32B(preview_width) * preview_height,
				(void*)preview_addr_vir + ALIGN_32B(preview_width) * preview_height * 5 / 4,
				ALIGN_32B(preview_width) * preview_height / 4);

		// v
		memcpy(img + ALIGN_32B(preview_width) * preview_height * 5 / 4,
				(void*)preview_addr_vir + ALIGN_32B(preview_width) * preview_height,
				ALIGN_32B(preview_width) * preview_height / 4);
	}
	else
	{
		...
		for (int h = 0; h < preview_height/2; h++)
		{
			...
			dst += ALIGN_16B(preview_width/2);
		}
		
		// u
		dst = (char *)img + ALIGN_4K(ALIGN_16B(preview_width)*preview_height) + ALIGN_4K(ALIGN_16B(preview_width)*preview_height/4);
		for (int h = 0; h < preview_height/2; h++)
		{
			memcpy(dst, src, preview_width/2);
			src += ALIGN_16B(preview_width/2);
