...
#define INPUT_SYSFS_PATH_GYRO "/sys/devices/platform/sun4i-i2c.1/i2c-1/1-006a/" //"/sys/class/i2c-adapter/i2c-0/0-0068/"
      ...
      mHasPendingEvent(false)
{
    ...
    ALOGD("GyroSensor::enable path=%s", input_sysfs_path);    
    } else {
    	ALOGE("%s: data_fd=[%d]\n", __func__, data_fd);
    }
}
...
int GyroSensor::enable(int32_t handle, int en) {
    if (en != mEnabled) {
        ...
        ALOGD("GyroSensor::enable(en=%d) : fd=%d, path=%s", en, fd, input_sysfs_path);
        if (fd >= 0) {
            char buf[2];
            int err;
            buf[1] = 0;
...
{
    ALOGD("GyroSensor::setDelay(handle=%d, ns=%d)");
    int fd;
