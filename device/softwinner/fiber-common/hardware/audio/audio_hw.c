...
static int digital_audio_start = 0;
enum digital_audio_method digital_method = 0;
...
#define MIXER_MIC1_BOOST_AMP_GAIN_CONTROL				"MIC1 boost AMP gain control"
#define MIXER_MIC2_BOOST_AMP_GAIN_CONTROL				"MIC2 boost AMP gain control"
...
#define BLUETOOTH_VOICE      		"mediasw.bluetooth"
...
struct mixer_ctls
{
	...
	struct mixer_ctl *mic1_boost_amp_gain_control;
	struct mixer_ctl *mic2_boost_amp_gain_control;
    ...
    struct mixer_ctl *audio_phone_endcall;
};
...
static int start_call(struct sunxi_audio_device *adev)
{
    ...
    mixer_ctl_set_value(adev->mixer_ctls.audio_spk_switch, 0, 0);
...
static void end_call(struct sunxi_audio_device *adev)
{
	int bt_on = 0;
    F_LOG;
    ALOGV("end_call, bluetooth_nrec=%d", adev->bluetooth_nrec);

	//property_set(BLUETOOTH_VOICE, "off");
	if (digital_audio_start) {
		sunxi_digital_audio_stop();
	}

	mixer_ctl_set_value(adev->mixer_ctls.audio_phone_endcall, 0, 1);
...
static void select_output_device(struct sunxi_audio_device *adev)
{
   ...
   // int pa_should_on=0;
    ...
    headset_on = adev->devices & AUDIO_DEVICE_OUT_WIRED_HEADSET;  // with mic
    headphone_on = adev->devices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE; // no mic
	...
	if (ret > 0)
	{
		ALOGV("get property audio.without.earpiece: %s", prop_value);
		if (strcmp(prop_value, "true") == 0)
		{
			//pa_should_on |= earpiece_on;
			speaker_on  |= earpiece_on;F_LOG
			earpiece_on = 0;
		}
	}
#endif
	...
	if (adev->mode == AUDIO_MODE_IN_CALL)
	{
			//mixer_ctl_set_value(adev->mixer_ctls.audio_adc_phone_in, 0, 0);
			//mixer_ctl_set_value(adev->mixer_ctls.audio_dac_phone_out, 0, 0);
#if 0
static int cur_dev = -1;
		if (cur_dev !=  adev->devices){
			cur_dev = adev->devices;
			ALOGV("*******select_output_device, devices: change %x", adev->devices);
		} else {
			return ;
		}
#endif
   		mixer_ctl_set_value(adev->mixer_ctls.audio_phone_in, 0,1);

		if(earpiece_on)	{
			mixer_ctl_set_value(adev->mixer_ctls.audio_phone_out, 0,1);
			mixer_ctl_set_value(adev->mixer_ctls.audio_earpiece_out, 0, 1);
			mixer_ctl_set_value(adev->mixer_ctls.audio_phone_main_mic, 0, 1);
		...
		} else if (headset_on) {  //mic
			mixer_ctl_set_value(adev->mixer_ctls.audio_phone_out, 0,1);
			mixer_ctl_set_value(adev->mixer_ctls.audio_headphone_out, 0, 1);
			mixer_ctl_set_value(adev->mixer_ctls.audio_phone_headset_mic, 0, 1);

			ALOGV("in wire head with mic ****LINE:%d,FUNC:%s",__LINE__,__FUNCTION__);
		} else if (bt_on) {
			if(digital_method == DIGITAL_AUDIO_METHOD_1){
			    mixer_ctl_set_value(adev->mixer_ctls.audio_phone_endcall, 0, 1);
			}else if(digital_method == DIGITAL_AUDIO_METHOD_2){
				...
				mixer_ctl_set_value(adev->mixer_ctls.audio_phone_headset_mic, 0, 0);
				mixer_ctl_set_value(adev->mixer_ctls.audio_phone_main_mic, 0, 0);

    			mixer_ctl_set_value(adev->mixer_ctls.audio_phone_in, 0,1);
    			mixer_ctl_set_value(adev->mixer_ctls.audio_phone_out, 0,1);

    			mixer_ctl_set_value(adev->mixer_ctls.audio_adc_phone_in, 0, 1);
    			mixer_ctl_set_value(adev->mixer_ctls.audio_dac_phone_out, 0, 1);
			}

			//property_set(BLUETOOTH_VOICE, "on");
            sunxi_digital_audio_start();
            digital_audio_start = 1;
		}

		//mixer_ctl_set_value(adev->mixer_ctls.audio_spk_switch, 0, (pa_should_on ? 1 : 0));
		//ALOGV("in no call mode, speaker,****LINE:%d,FUNC:%s,pa:%d",__LINE__,__FUNCTION__,pa_should_on);

		set_incall_device(adev);
	}
	else
	{
		mixer_ctl_set_value(adev->mixer_ctls.audio_spk_switch, 0, (speaker_on ? 1 : 0));
		ALOGV("in no call mode, speaker,****LINE:%d,FUNC:%s",__LINE__,__FUNCTION__);
	}
...
static void select_input_device(struct sunxi_audio_device *adev)
{
    ...
    voice_record = adev->devices & AUDIO_DEVICE_IN_VOICE_CALL;

    if (adev->mode == AUDIO_MODE_IN_CALL) {
	mixer_ctl_set_value(adev->mixer_ctls.audio_phone_voice_record, 0, 1);
	if (earpiece_on | speaker_on | headphone_on ){
		ALOGV("earpiece_on | speaker_on | headphone_on,****LINE:%d,FUNC:%s",__LINE__,__FUNCTION__);

	} else if (headset_on) {
		ALOGV("headset_on,****LINE:%d,FUNC:%s",__LINE__,__FUNCTION__);
...
//        }
	...
	out_frames = in_frames;
	 buf = (void *)buffer;
	//ALOGV("***line:%d,LEN:%d",__LINE__,out_frames * frame_size);
...
static int start_input_stream(struct sunxi_stream_in *in)
{
	...
	if (adev->mode == AUDIO_MODE_IN_CALL)
	{
	    ...
	    mixer_ctl_set_value(adev->mixer_ctls.audio_phone_voice_record, 0, 1);F_LOG;
                                   ...
                                   struct audio_stream_in *stream)
{
    ...
    mixer_ctl_set_value(ladev->mixer_ctls.audio_phone_voice_record, 0, 0);
        ALOGE("adev_close_input_stream set voice record status 0 ");
    return;
}
                     ...
                     hw_device_t** device)
{
	...
	adev->mixer_ctls.audio_phone_out = mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_PHONE_OUT);
	if (!adev->mixer_ctls.audio_phone_out) {
		ALOGE("Unable to find '%s' mixer control",MIXER_AUDIO_PHONE_OUT);
		goto error_out;
	}

	adev->mixer_ctls.audio_phone_in = mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_PHONE_IN);
	if (!adev->mixer_ctls.audio_phone_in) {
		ALOGE("Unable to find '%s' mixer control",MIXER_AUDIO_PHONE_IN);
		goto error_out;
	}

	adev->mixer_ctls.audio_earpiece_out = mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_EARPIECE_OUT);
	if (!adev->mixer_ctls.audio_earpiece_out) {
		ALOGE("Unable to find '%s' mixer control",MIXER_AUDIO_EARPIECE_OUT);
		goto error_out;
	}

	adev->mixer_ctls.audio_headphone_out = mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_HEADPHONE_OUT);
	if (!adev->mixer_ctls.audio_headphone_out) {
	...
	adev->mixer_ctls.audio_adc_phone_in= mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_ADC_PHONE_IN);
	if (!adev->mixer_ctls.audio_adc_phone_in) {
		ALOGE("Unable to find '%s' mixer control", MIXER_AUDIO_ADC_PHONE_IN);
		goto error_out;
	}

	adev->mixer_ctls.audio_dac_phone_out= mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_DAC_PHONE_OUT);
	if (!adev->mixer_ctls.audio_dac_phone_out) {
		ALOGE("Unable to find '%s' mixer control", MIXER_AUDIO_DAC_PHONE_OUT);
		goto error_out;
	}

	adev->mixer_ctls.audio_phone_main_mic= mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_PHONE_MAIN_MIC);
	if (!adev->mixer_ctls.audio_phone_main_mic) {
		ALOGE("Unable to find '%s' mixer control", MIXER_AUDIO_PHONE_MAIN_MIC);
		goto error_out;
	}

	adev->mixer_ctls.audio_phone_headset_mic= mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_PHONE_HEADSET_MIC);
	if (!adev->mixer_ctls.audio_phone_headset_mic) {
		ALOGE("Unable to find '%s' mixer control", MIXER_AUDIO_PHONE_HEADSET_MIC);
		goto error_out;
	}

	adev->mixer_ctls.audio_phone_voice_record= mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_PHONE_VOICE_RECORDER);
	if (!adev->mixer_ctls.audio_phone_voice_record) {
	        ALOGE("Unable to find '%s' mixer control", MIXER_AUDIO_PHONE_VOICE_RECORDER);
	        goto error_out;
	}

	adev->mixer_ctls.audio_spk_switch = mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_SPK_SWITCH);
	if (!adev->mixer_ctls.audio_spk_switch) {
	        ALOGE("Unable to find '%s' mixer control",MIXER_AUDIO_SPK_SWITCH);
	        goto error_out;
	}

	adev->mixer_ctls.audio_phone_endcall= mixer_get_ctl_by_name(adev->mixer, MIXER_AUDIO_PHONE_ENDCALL);
	if (!adev->mixer_ctls.audio_phone_endcall) {
