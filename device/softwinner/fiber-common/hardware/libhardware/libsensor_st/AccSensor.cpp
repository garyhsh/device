...
#define INPUT_SYSFS_PATH_ACC "/sys/class/i2c-adapter/i2c-1/1-001e/accelerometer/"

/*****************************************************************************/

AccSensor::AccSensor()
      : SensorBase(NULL, "lsm303d_acc"),
      //mEnabled(0),
      mInputReader(4),
      mEnCount(0)
{
...
int AccSensor::enable(int32_t, int en) {
...
{
    int fd;
    strcpy(&input_sysfs_path[input_sysfs_path_len], "pollrate_us");
    fd = open(input_sysfs_path, O_RDWR);
    if (fd >= 0) {
        char buf[80];
        sprintf(buf, "%lld", delay_ns/1000);
	ALOGD("%s ++++++++++++++++++=SET delay is %lld", input_sysfs_path,delay_ns);
