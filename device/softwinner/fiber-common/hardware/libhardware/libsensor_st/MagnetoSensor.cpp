...
#define INPUT_SYSFS_PATH_MAGNETO "/sys/class/i2c-adapter/i2c-1/1-001e/magnetometer/"
...
int MagnetoSensor::enable(int32_t, int en) {
    ...
    if (flags != -1) {
        ...
        if (fd >= 0) {
			ALOGD("%s 111+++++++++++++++++=SET delay is %d", input_sysfs_path,en);
            char buf[2];
            int err;
            buf[1] = 0;
            if (flags) {
...
int MagnetoSensor::setDelay(int32_t handle, int64_t delay_ns)
{
    ...
    if (fd >= 0) {
        ...
        char buf[80];
        sprintf(buf, "%d", (int)(delay_ns / 1000));
